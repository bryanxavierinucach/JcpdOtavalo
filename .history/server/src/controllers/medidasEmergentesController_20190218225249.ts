...
class MedidasEmergentesController{
    ...
    public async list(req: Request, res:Response):Promise<void> {
        const medidasEmergentes = await pool.query('SELECT * FROM t_medidasEmergentes');
         res.json(medidasEmergentes);
    }
    //Listar solo un medidasEmergentes por el ID
    public async getOne(req: Request, res:Response): Promise<any> {
        const {id} = req.params;
        const medidasEmergentes = await pool.query('SELECT * FROM t_medidasEmergentes WHERE avo_id = ? ', [id]);
